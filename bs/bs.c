...
static ship_t plyship[SHIPTYPES] = { { carrier, 0, 'A', 5, 0, 0, E, FALSE }, {
		battle, 0, 'B', 4, 0, 0, E, FALSE }, { destroy, 0, 'D', 3, 0, 0, E,
FALSE }, { sub, 0, 'S', 3, 0, 0, E, FALSE },
		{ ptboat, 0, 'P', 2, 0, 0, E, FALSE }, };

static ship_t cpuship[SHIPTYPES] = { { carrier, 0, 'A', 5, 0, 0, E, FALSE }, {
		battle, 0, 'B', 4, 0, 0, E, FALSE }, { destroy, 0, 'D', 3, 0, 0, E,
FALSE }, { sub, 0, 'S', 3, 0, 0, E, FALSE },
		{ ptboat, 0, 'P', 2, 0, 0, E, FALSE }, };
...
static void announceopts(void) {
	printw("Playing %s game (",
			(salvo || blitz || closepack) ? "optional" : "standard");
...
static void initgame(void) {
	...
	mvprintw(HYBASE + 5, HXBASE, "place all remaining ships randomly.");
...
static int getcoord(int atcpu) {
	...
	for (;;) {
		if (atcpu) {
			mvprintw(CYBASE + BDEPTH + 1, CXBASE + 11, "(%d, %c)", curx,
					'A' + cury);
			cgoto(cury, curx);
		} else {
			mvprintw(PYBASE + BDEPTH + 1, PXBASE + 11, "(%d, %c)", curx,
					'A' + cury);
			pgoto(cury, curx);
		}
...
static bool cpufire(int x, int y) {
	...
	mvprintw(PROMPTLINE, 0, "I shoot at %c%d. I %s!", y + 'A', x,
			hit ? "hit" : "miss");
	ss = hitship(x, y);
	sunk = hit && ss;
	if (sunk)
...
int playagain(void) {
	...
	mvprintw(1, (COLWIDTH - j) / 2, "%s: %d     Computer: %d", name, plywon,
			cpuwon);

	prompt(2,
			(awinna()) ?
					"Want to be humiliated again, %s [yn]? " :
